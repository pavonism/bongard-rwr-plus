...
def make_instance(problem_id: int = 1, side=Side.LEFT):
    return DescriptionToSideExperimentInstance(
        problem_id=problem_id,
        descriptions=make_problem_descriptions(),
        response=DescriptionToSideResponse(
            concept="concept", answer=Side.LEFT, explanation="explanation"
        ),
...
def test_create_request(expected_answer: Side):
    descriptions = make_problem_descriptions()
    request = descriptions.create_request(expected_answer)

    test_description = (
        descriptions.left_descriptions
        if expected_answer == Side.LEFT
...
def test_evaluate():
    ...
    instance.response.answer = Side.RIGHT
...
def test_has_problem_solution():
    ...
    instance = make_instance()
    experiment.add_solution(instance)
    assert experiment.has_problem_solution(make_problem(), instance.expected_answer)


def test_to_file_and_from_file():
    source = DescriptionToSideExperiment(solver_name="dummy", descriptor_name="Author")
    source.add_solution(make_instance(problem_id=1, side=Side.LEFT))
    source.add_solution(make_instance(problem_id=2, side=Side.RIGHT))
    with tempfile.NamedTemporaryFile(delete=False) as f:
        source.to_file(f.name)
        f.seek(0)
        target = DescriptionToSideExperiment.from_file(f.name)
    print(target)
    assert target.has_problem_solution(make_problem(1), Side.LEFT)
    assert target.has_problem_solution(make_problem(2), Side.RIGHT)
    assert target.has_problem_solution(make_problem(1), Side.RIGHT)
    assert target.has_problem_solution(make_problem(2), Side.LEFT)
    assert not target.has_problem_solution(make_problem(3), Side.LEFT)
